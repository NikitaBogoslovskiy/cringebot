...
namespace NeuralNetwork1
{
    public static class ImageEditor
    {
        ...
        public static Bitmap Augment(Bitmap input)
        {
            ...
            //int length = input.Height;
            //float scale_factor = (float)((r.NextDouble() * 0.3 + 0.8));
            //int new_length = (int)(scale_factor * length);
            //int left_x, bottom_y;
            //left_x = bottom_y = (length / 2) - (new_length / 2);
            //int shift_limit = (length - new_length) / 2;
            //int x_shift = (int)((float)(r.NextDouble() * 2 - 1) * shift_limit);
            //int y_shift = (int)((float)(r.NextDouble() * 2 - 1) * shift_limit);
            //var augmented_image = new Bitmap(input.Width, input.Width);
            //using (var g = Graphics.FromImage(augmented_image))
            //{
            //    using (ImageAttributes attributes = new ImageAttributes())
            //    {
            ...
            //        g.PixelOffsetMode = PixelOffsetMode.HighQuality;
            //        g.Clear(Color.White);
            //        Rectangle section = new Rectangle(new Point(left_x + x_shift, bottom_y + y_shift), new Size(new_length, new_length));
            //        g.DrawImage(rotated_image, section, 0, 0, input.Width, input.Height, GraphicsUnit.Pixel, attributes);
            //    }
            //}
